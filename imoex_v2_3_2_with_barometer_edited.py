...
class SignalNotifier:
    ...
    def _send(self, subject, body_html, body_text, attachments=None):
        ...
        try:
            ...
            # Тело письма (alternative: plain + html)
    ...
    def send_status_report(self, date, cagr, max_dd, sharpe, current_signal, 
        ...
        # Динамический вывод барометра для email
        barometer_conclusion = ""
        if barometer_data:
            net_jur = barometer_data.get('index_net_jur', 0)
            jur_lr = barometer_data.get('index_jur_long_ratio', 50)
            net_fiz = barometer_data.get('index_net_fiz', 0)
            fiz_lr = barometer_data.get('index_fiz_long_ratio', 50)
            jur_side = "шорте" if net_jur < 0 else "лонге"
            fiz_side = "лонге" if net_fiz >= 0 else "шорте"
            smart_action = "хеджируют/играют на понижение" if net_jur < 0 else "накапливают позиции"
            crowd_action = "покупает просадку" if net_fiz >= 0 and net_jur < 0 else "продает на росте" if net_fiz < 0 and net_jur >= 0 else "настроена нейтрально"
            barometer_conclusion = (
                f"Вывод: Юрлицы преимущественно в {jur_side} (нетто {net_jur:+.1f}%, доля лонгов {jur_lr:.1f}%), "
                f"физлицы — в {fiz_side} (нетто {net_fiz:+.1f}%, доля лонгов {fiz_lr:.1f}%). "
                f"Это классическая картина для фазы {phase_name} — «умные деньги» {smart_action}, "
                f"толпа {crowd_action}."
            )
        barometer_disclaimer = "Барометр не влияет на торговые решения — только для подтверждения контекста."
...
"""
        ...
        # Формируем CSV-вложение с данными барометра
        attachments = []
        if barometer_df is not None and not barometer_df.empty:
            # Custom ticker order: ascending by index weight, SBRF before MXI at the very end
            # Weights: ALRS 0.02, PLZL 0.03, VTBR 0.03, MGNT 0.04, NVTK 0.05, ROSN 0.05,
            #          GMKN 0.06, YDEX 0.07, GAZR 0.10, LKOH 0.14, SBRF 0.16, MXI
            custom_order = ['ALRS', 'PLZL', 'VTBR', 'MGNT', 'NVTK', 'ROSN',
                            'GMKN', 'YDEX', 'GAZR', 'LKOH', 'SBRF', 'MXI']

            # Sort data: by custom ticker order, then by date within each ticker
            barometer_df['_sort_key'] = barometer_df['asset'].map(
                {t: i for i, t in enumerate(custom_order)}
            )
            barometer_df = barometer_df.sort_values(['_sort_key', 'date']).drop(columns=['_sort_key'])

            # Create a copy and add row numbering
            df_export = barometer_df.copy()
            df_export.insert(0, '№', range(1, len(df_export) + 1))

            # Calculate summary per ticker: count and yur_share change
            summary_rows = []
            summary_rows.append({})  # empty row separator
            summary_rows.append({'№': '', 'date': '=== ОТЧЁТ ПО ТИКЕРАМ ===', 'asset': '', 'type': '',
            ...
            for asset in custom_order:
                ...
                count = len(asset_df)
                if count >= 2:
                    last_yur = asset_df['yur_share'].iloc[-1]
                    prev_yur = asset_df['yur_share'].iloc[-2]
                    delta = last_yur - prev_yur
                    delta_str = f"{delta:+.2f}%"
                elif count == 1:
                    last_yur = asset_df['yur_share'].iloc[0]
                ...
                summary_rows.append({
                    '№': '', 'date': '', 'asset': asset, 'type': f'Записей: {count}',
                    'total_oi': '', 'fiz_oi': '', 'yur_oi': '',
                    'fiz_share': '', 'yur_share': f'{last_yur:.2f}% ({delta_str})',
                    'long_fiz': '', 'short_fiz': '', 'long_jur': '', 'short_jur': ''
                })
            ...
            for col in df_export.columns:
                if col not in summary_df.columns:
                    summary_df[col] = ''
            summary_df = summary_df[df_export.columns]
            ...
            attachments.append((f"barometer_{last_trade_date}.csv", csv_bytes))
...
class MOEXBarometerLayer:
    ...
    def get_current_metrics(self):
        ...
        return {
            ...
            'index_net_jur': index_net_jur, 'index_net_fiz': index_net_fiz,
            'index_jur_long_ratio': index_jur_long_ratio, 'index_fiz_long_ratio': index_fiz_long_ratio,
            'covered_weight': covered_weight, 'smart_delta_10d': norm_smart_delta,
...
def run_strategy(df, notifier=None, ema_fast=15, ema_slow=50, ma_long=126,
    ...
    if notifier and notifier.enabled:
        notifier.send_status_report(
            ...
            barometer_df=barometer_layer.df if barometer_layer else None
        )
...
if __name__ == "__main__":
    ...
    if result.get('barometer'):
        ...
        print(f"  Δ доли Юрлиц (10д): {b['smart_delta_10d']:+.2f}%")
        print(f"  Доля Юрлиц (всего): {b['index_smart_control']:.1f}%")
        print(f"    └─ Нетто-позиция: {net_jur:+.1f}% ({'лонг' if net_jur >= 0 else 'шорт'})")
        print(f"    └─ Доля лонгов:   {jur_lr:.1f}%")

        print(f"  Физлицы:")
        print(f"    └─ Нетто-позиция: {net_fiz:+.1f}% ({'лонг' if net_fiz >= 0 else 'шорт'})")
        print(f"    └─ Доля лонгов:   {fiz_lr:.1f}%")
